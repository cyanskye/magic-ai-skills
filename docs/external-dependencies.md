...
本仓库里的 Skills 不独立完成所有工作。它们依赖输入源和目标平台。Get 笔记是典型输入源；Obsidian、ima 等知识库是目标平台。
...
`magic-kb-compiler` 可以把这些材料编译进 Magic AI 知识库。
...
`magic-kb-compiler` 的核心能力是把原始材料编译成可复用知识资产。目标平台可以是 Obsidian / Magic AI 知识库，也可以是 ima 或其他知识库。
...
ima 这类平台可以作为目标平台。它们更像“导入/检索/问答空间”，不一定适合原样承载 Obsidian 的 raw/cards/wiki/views/logs 文件结构。
...
如果平台提供 API 或官方导入能力，可以再补自动化发布脚本。没有稳定 API 时，Skill 应先生成可导入文件包和操作说明，不要假装已经完成平台写入。

### 重要边界

本仓库不保存 Obsidian vault 内容，也不保存 ima 或其他平台中的知识库内容。
